...
Mesh *Obj::init(const char * filename){
    ...
    std::vector< unsigned int > temp_normalIndices, indices;
    ...
    std::cout << "done." << std::endl;
    // given vertices, normals, indices
    return new Mesh(vertices, normals, indices);
}

void Obj::subdiv(GLuint subdivLevel, Mesh *mesh) {    
    // int subdivLevel = 2;

    std::cout << "Subdiving: level " << subdivLevel << "..." ; 

    mesh->Subdiv(subdivLevel);// (make halfedges, faces, verticies. and turn them into vertices, normals, indices)
    std::vector< glm::vec3 > temp_vertices, vertices;
    std::vector< glm::vec3 > temp_normals, normals;
    std::vector< unsigned int > temp_vertexIndices;
    std::vector< unsigned int > temp_normalIndices, indices;
    temp_vertices = mesh->getVertices();
    temp_normals = mesh->getNormals();
    std::vector<GLuint> temp_indices = mesh->getIndices();
    
    GLuint n = temp_indices.size(); // #(triangles)*3
    vertices.resize(n);
    normals.resize(n);
    indices.resize(n);
    for (unsigned int i = 0; i<n; i++){
        indices[i] = i;
        vertices[i] = temp_vertices[ temp_indices[i] ];
        normals[i] = temp_normals[ temp_indices[i] ];
    }
    std::cout << "done." << std::endl;   
